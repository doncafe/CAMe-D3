...
def extract_swdown_timeseries(wrf_dir, point_lat=None, point_lon=None):
    ...
    wrf_files = sorted(glob.glob(os.path.join(wrf_dir, "wrfout_d01_*.nc")))
...
def plot_swdown_timeseries(df, daily_stats):
    ...
    plt.tight_layout()
    plt.savefig('swdown_timeseries.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    if df is not None:
        ...
        df.to_csv("swdown_hourly_timeseries.csv", index=False)
        daily_stats.to_csv("swdown_daily_statistics.csv")
